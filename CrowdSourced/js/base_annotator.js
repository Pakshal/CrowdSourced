...
function call() 
{
	...
	//and annotationvalue is the name of textarea having the annotation
          ...
          type = document.forms["annotation"]["type"].value;
            if(type==""||type==null)
            {
        ...
        //checking if something is there in the textarea or not($.trim is used 
        //for validating textarea) 
        if($.trim(annotat)!=null && $.trim(annotat)!="")
		{
            
            //concatinating the annotation category tag to the annotation
            //based on the selected category
            //ex:[Q] if the question category is selected
            

            annotat=type.concat(annotat);
...
function alertSelection() {
    ...
    var val=selc.range.toString(); //selc is a global variable which contains start and end value of selected text, range is slected text
    selectAndHighlightRange('maintext', selc.start,selc.end,11);//maintext is id of div of main content, 
...
//ajax code
 ...
 //dbinsert_anno.inc.php is a file which inserts the text and annotation values etc in the table content
