...
$result=mysqli_query($link,"select * from ansvotes");
...
while($row=mysqli_fetch_array($result)){ 
	fputcsv($handle, array($row['id'], $row['ansid'],$row['user'],$row['votetype'])); 
}   
...
$handle = fopen($filename, 'w+');   
// Write the spreadsheet column titles / labels 
fputcsv($handle, array('id','text','annotation','votes','date','spos','epos','user','article'));   
// Write all the user records to the spreadsheet 
while($row=mysqli_fetch_array($result)){ 
	fputcsv($handle, array($row['id'], $row['text'],$row['annotation'],$row['votes'],$row['date'],$row['spos'],$row['epos'],$row['user'],$row['article'])); 
}   
// Finish writing the file 
fclose($handle);   
...
$filename = $name."/passiveusers.csv";
$result=mysqli_query($link,"select * from passiveusers");
$handle = fopen($filename, 'w+');   
// Write the spreadsheet column titles / labels 
fputcsv($handle, array('annoid','user','timestamp','articlename'));   
// Write all the user records to the spreadsheet 
while($row=mysqli_fetch_array($result)){ 
  fputcsv($handle, array($row['annoid'], $row['user'],$row['date'],$row['articlename'])); 
}   
// Finish writing the file 
fclose($handle);
